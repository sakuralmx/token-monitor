...
async function refreshCatalogView() {
  ...
  try {
    ...
    // back to the local scan when the hub is unreachable / has no catalog.
    const result = await window.tokenMonitor.getHubCatalogEntries?.();
    if (!result) return;
