...
`quotaPercentageHistory` is optional authenticated audit history. It is keyed independently by `codex` and `opencode`, then by `accounts[accountKey]`; each account contains `updatedAt` and its complete retained observation timeline. Every observation contains the official `remainingPercent`, an ISO `at` timestamp, optional `resetsAt`, and sanitized cumulative input/cache-read/cache-write/output components. Consecutive observations with the same percentage are compacted to the first and latest confirmation of that plateau; intermediate unchanged polls are discarded, while both plateau bounds and every percentage transition remain. There is no record-count cap. The OpenCode entry is recorded only for Go accounts and its components come only from usage attributed to `provider=opencode-go`. These values are not used to estimate token capacity, remaining tokens, weights, cost, or time-to-exhaustion.

New clients never put the complete history in ordinary `/api/ingest`: it is stripped on every payload-build and retry path so normal usage/limits synchronization remains bounded. Instead, clients split pending observations into batches of at most 200 and send `{ history }` to authenticated `POST /api/quota-history`; authenticated `GET /api/quota-history` returns the Hub-scoped provider/account history. Node Hub and Worker merge batches idempotently by provider, account and observation timestamp. A successful-upload cursor records individual timestamps rather than a maximum timestamp, so a late older observation is still uploaded. For compatibility, Hubs accept an old device payload containing `quotaPercentageHistory`, import it into Hub-scoped history, strip it before saving the device record, and roll both changes back if persistence fails. Neither authenticated `/api/stats` nor public stats carries the complete history; callers retrieve it explicitly from the authenticated history route.
