...
`quotaPercentageHistory` is optional authenticated device-only audit history. It is keyed independently by `codex` and `opencode`; each provider entry contains a hashed `accountKey`, `updatedAt`, and the complete retained observation timeline. Every observation contains the official `remainingPercent`, an ISO `at` timestamp, optional `resetsAt`, and sanitized cumulative input/cache-read/cache-write/output components. Consecutive observations with the same percentage are losslessly compacted to the first and latest confirmation of that plateau; intermediate unchanged polls are discarded, while both plateau bounds and every percentage transition remain. There is no record-count cap. The OpenCode entry is recorded only for the Go account and its components come only from usage attributed to `provider=opencode-go`. These values are not used to estimate token capacity, remaining tokens, weights, cost, or time-to-exhaustion. Limits-only and partial updates merge providers independently so updating Codex cannot erase OpenCode history. The field is returned only inside authenticated `devices[]`; public stats omit the complete device list.
