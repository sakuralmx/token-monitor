...
## 协作开发约定（dev-flow，个人工作流附加层）

> 本节是用户个人化的多工具接力流程，叠加在上面的官方规范之上；冲突时以官方规范为准。

### 短指令（用户说出时直接执行，不要反问）

每条短指令分「默认版」和「插件版」：只说短指令走默认版（纯文件流程，不调用插件）；说「用 XX 插件」走插件版（结果必须落地成 docs/ 文件并 commit）。

- 「开始规划」→ 读当前项目状态，写 docs/plan.md（目标/技术栈/任务清单带验收标准/风险点），commit。
- 「拆任务」
  · 默认版：读 docs/plan.md，写 docs/tasks.md（任务 id/描述/依赖/验收标准），commit。
  · 插件版（All in Luna）：仅在 allinflash profile 会话中可用（web 会话无其工具），任务图原样抄进 docs/tasks.md，commit。
- 「接手执行」
  · 默认版：读 docs/plan.md、tasks.md、handoff.md（若有），逐任务实现，每任务 commit（message 含任务 id），完成后更新 handoff.md 并 commit。
  · 插件版（All in Luna）：仅在 allinflash profile 会话中可用，结果原样抄进 handoff.md，commit。
- 「你来审查」
  · 默认版：读 docs/plan.md、handoff.md、最近 git diff，逐任务对照验收标准找 bug（逻辑/边界/性能/安全），写 docs/review.md（严重/一般/建议分级 + 复现 + 修复建议），小问题直接修并 commit。
  · 插件版：用 dsh-agent-teams 组异源审查团队（成员各配不同 provider/model），让每个成员各自调用 dsh-inspect 的 checkup/review 做对抗式审查，队长汇总后写进 docs/review.md，commit。审查范围由用户决定，默认本次更新内容（首次即全量）。
- 「收工交接」→ 更新 docs/handoff.md，把上一轮 handoff/review/tasks 归档到 docs/archive/（<日期>-<功能>.md），commit。

### 落地铁律（插件版专用）

插件产出是中间产物，不是交接文件：dsh-agent-teams 产出 `.agent-teams/` 目录、dsh-inspect 产出对话内报告、All in Luna 产出独立 profile 的 run state。落地规则（防失真）：**原样转存，不擅自提炼**——问题清单逐条、复现方式、修复建议、结论逐字写进 docs/ 对应 md 文件，不概括删减；原始产物目录（如 `.agent-teams/`）保留并 git 提交供回查。

### 模型分工（提示）

- GPT（Codex 桌面端）：规划、疑难 bug、拍板建议（贵，量小）
- DeepSeek V4 Pro（DSH）：开发主力（便宜）
- DeepSeek V4 Flash（DSH）：拆任务、开发中快检（最便宜）
- Claude / Grok / GPT（意心中转）：异源审查（贵）

### 个人流程铁律

1. 任何操作前 git status，工作树不干净先处理。
2. 开工先读 docs/ 下交接文件；上下文靠文件传递。
3. 同一时间只让一个工具改代码（串行接力）。
4. 每轮结束必须 commit；commit message 遵循上方「Commit messages」的 conventional-commit 格式。
5. 审查范围由用户决定，默认审查本次更新内容（首次即全量）；审查目标是合格（无 bug、无错误），不以省成本为优先。

### 归档约定

docs/ 下永远是「当前一轮」的文件；历史 handoff/review/tasks 移入 docs/archive/（<日期>-<功能>.md）。plan.md 长期演进不归档。归档文件只是磁盘历史，任何会话按需读取，不自动进上下文。
