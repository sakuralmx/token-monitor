...
test('uploadCatalogDelta posts batches and aggregates counts', async () => {
  ...
  assert.deepEqual(result, { accepted: 2, invalidated: 1, unavailable: false, catalogVersion: 1 });
...
test('uploadCatalogDelta treats catalog_unavailable as a silent downgrade, not a failure', async () => {
  ...
  assert.deepEqual(result, { accepted: 0, invalidated: 0, unavailable: true, catalogVersion: 0 });
});
